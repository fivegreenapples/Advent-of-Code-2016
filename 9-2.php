...
function countForSection($section)
{
    ...
    for ($c=0; $c<strlen($section); $c++) {
        ...
        if ($mode == "normal") {
            if ($char === "(") {
                $mode = "need-numberchars";
                $numberChars = "";
                $numberReps = "";
            } else {
                $count += 1;
            }
        } elseif ($mode == "need-numberchars") {
            if ($char === "x") {
                $mode = "need-numberrepititions";
                $numberChars = intval($numberChars);
            } else {
                $numberChars .= $char;
            }
        } elseif ($mode == "need-numberrepititions") {
            if ($char === ")") {
                $numberReps = intval($numberReps);
                $subSection = substr($section, $c+1, $numberChars);
                $count += ($numberReps * countForSection($subSection));
                $c += $numberChars;
                $mode = "normal";
            } else {
                $numberReps .= $char;
...
function run($in) {
    ...
    $res = countForSection($in);

    echo "\n\nUncompressed length: ".$res."\n\n";
