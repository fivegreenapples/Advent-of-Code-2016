...
(function() {
	...
	let MOVE = (currentDirection, length, easting, northing) => {
		switch(DIRECTIONS[currentDirection]) {
			case "N":
				return [easting, northing+length]
			case "E":
				return [easting+length, northing]
			case "S":
				return [easting, northing-length]
			case "W":
				return [easting-length, northing]
	...
	INPUT.split(' ').forEach( instruction => {
		...
		[currentEasting, currentNorthing] = MOVE(currentDirection, count, currentEasting, currentNorthing)
