...
Your puzzle input is still 01000100010010111.
...
// $TARGET_DISK = 272;
// part 2
$TARGET_DISK = 35651584;

// $OUTPUT = $TEST_INPUT;
// $TARGET = $TEST_TARGET_DISK;
$OUTPUT = $INPUT;
$TARGET = $TARGET_DISK;

while(strlen($OUTPUT) < $TARGET) {

	$initial = $OUTPUT;
	$OUTPUT .= "0";
	for($c=strlen($initial)-1; $c>=0; --$c) {
		$OUTPUT .= $initial[$c]==="1" ? "0" : "1";
	}
}

$OUTPUT = substr($OUTPUT, 0, $TARGET);

while(strlen($OUTPUT) % 2 === 0) {
	$initial = $OUTPUT;
	$OUTPUT = "";
	for($c=0; $c<strlen($initial); $c+=2) {
		$OUTPUT .= $initial[$c]===$initial[$c+1] ? "1" : "0";
	}
}
