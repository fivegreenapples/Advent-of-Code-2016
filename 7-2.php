...
function findAbas($str) {
	...
	for ($i=2; $i < strlen($str); $i++) {
		...
		if ($char1 === $char3 && $char1 !== $char2) {
			$abas[] = $char1.$char2.$char3;
		}
	}
	return $abas;
}

function reverseAbas($abas) {
	$babs = [];
	foreach($abas as $aba) {
		$babs[] = $aba[1].$aba[0].$aba[1];
	}
	return $babs;
}

function run($addresses) {
	...
	foreach($addresses as $address) {
		...
		// this forces hypernet sequences into the odd numbered array indices
		$addressBits = preg_split("/[][]/", $address);
		$supernetAbas = [];
		$hypernetAbas = [];
		foreach($addressBits as $b => $bit) {
			if ($b % 2 === 0) {
				$supernetAbas = array_merge($supernetAbas, findAbas($bit));
			} else {
				$hypernetAbas = array_merge($hypernetAbas, findAbas($bit));
			}
		}
		$supernetAbas = array_unique($supernetAbas);
		$hypernetAbas = array_unique($hypernetAbas);
		$hypernetBabs = reverseAbas($hypernetAbas);

		if (array_intersect($supernetAbas, $hypernetBabs)) {
			echo $address."\n";
			$countSSLSupport += 1;
		}
	...
	echo "\n\nNumber of addresses that support SSL is: ".$countSSLSupport."\n\n"; 
