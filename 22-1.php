...
$REAL_INPUT = "root@ebhq-gridcenter# df -h
...
$INPUT = $REAL_INPUT;
function addNode(&$nodes, $dfLine, $x, $y, $cap, $used, $avail, $percentUsed) {
	if ($cap != $used + $avail) {
		die("\n\nError: Capacity mismatch: $dfLine\n\n");
	}
	if ($percentUsed != floor(100 * $used / $cap)) {
		die("\n\nError: Percent used mismatch: ".round(100 * $used / $cap)." $dfLine\n\n");
	}

	$nodes[$x][$y] = [$cap, $used, $avail];
}

$nodes = array_map("trim", explode("\n", trim($INPUT)));
$nodeDetails = [];
foreach($nodes as $node) {

	if (preg_match("|^/dev/grid/node-x([0-9]+)-y([0-9]+) +([0-9]+)T +([0-9]+)T +([0-9]+)T +([0-9]+)%$|", $node, $matches)) {
		addNode($nodeDetails, $node, $matches[1], $matches[2], $matches[3], $matches[4], $matches[5], $matches[6]);
	} else {
		echo ("Unhandled node: ".$node."\n");
	} 
}

$numViable = 0;
foreach ($nodeDetails as $x => $xNodes) {
	foreach ($xNodes as $y => $node) {
		$used = $node[1];

		foreach ($nodeDetails as $xx => $xxNodes) {
			foreach ($xxNodes as $yy => $nodenode) {
				$avail = $nodenode[2];
				if (($x!=$xx || $y!=$yy) && $used && $used <= $avail) {
					$numViable += 1;
				}
			}
		}
...
echo "\n\nNumber of viable nodes is $numViable\n\n";